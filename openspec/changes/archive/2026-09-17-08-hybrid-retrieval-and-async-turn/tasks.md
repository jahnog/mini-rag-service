...
- [x] 1.1 Create `src/bcra_rag/domain/text.py` with `SPANISH_STOPWORDS` (move the literal set from `src/bcra_rag/adapters/index_fake.py::search`, ~line 37-62) and
  ```python
...
- [x] 1.2 Create `src/bcra_rag/domain/bm25.py` with `Bm25Index.build(docs: Sequence[tuple[str, str]])`, `Bm25Index.search(query: str, n: int) -> list[tuple[str, float]]` (standard BM25, `k1=1.5`, `b=0.75`, IDF `math.log((N - df + 0.5) / (df + 0.5) + 1)`) and `rrf(rankings: Sequence[Sequence[str]], *, k: int = 60) -> list[tuple[str, float]]`.
- [x] 1.3 Tests `tests/test_bm25.py`: `tokenize("Comunicación A 3500 punto 8.4.1")` contains `"a3500"`, `"3500"`, `"punto"`, not `"de"`; BM25 on three docs ranks the doc containing a rare query term first; a query with no matching term returns `[]`; `rrf([["a", "b"], ["b", "c"]])` puts `"b"` first; ties keep first-appearance order. Verify: `uv run pytest tests/test_bm25.py -q` → passes.

## 2. retrieval — settings and Chroma adapter

- [x] 2.1 `src/bcra_rag/settings.py`, in the `# Retrieval` block: add
  ```python
...
- [x] 2.2 `src/bcra_rag/adapters/index_chroma.py`:
...
- [x] 2.3 Tests `tests/test_index.py`:
  ```python
...
- [x] 3.1 `src/bcra_rag/domain/manifest.py`: add
  ```python
