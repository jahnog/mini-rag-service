...
The production-smoke command SHALL request `GET /evals/l1` from the running public process and SHALL fail (not skip) when the response is not HTTP 200, when it lacks `retrieval`, `generation`, `judge`, or `n`, or when `unpublished` or `sample` is true. It MUST NOT run L1.

#### Scenario: Host serves a published run
- **GIVEN** the operator invoked the production-smoke command
- **WHEN** the client requests `GET /evals/l1`
...
- **WHEN** the production-smoke command requests `GET /evals/l1`
