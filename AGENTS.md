...
BCRA CAMEX only. Never Banxico. Five ports, no Redis, no Next.js in v1. Default tests are fakes; `src` coverage MUST be >= 80%. Do not spawn uvicorn from tests. Do not run `--run-live-server` or a paid L1 eval in CI.
