...
**Dump host.** After ingest on that host, run `./scripts/run-l1.sh` (same suite flags as the laptop CLI; placeholder `user@dump-host`). Serving is down for the run; in-process sessions do not survive. The API starts again on L1 failure so staff Calidad L1 reloads the stored document. The unpublished/sample banner follows the stored file, not the laptop vs dump-host invocation. `GET /l1` returns the document the running process serves.
